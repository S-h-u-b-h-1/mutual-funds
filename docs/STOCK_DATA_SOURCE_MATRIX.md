...
## Verified source baseline — 12 August 2026
...
- NSE's published Data Sharing & Usage Policy explicitly covers real-time, delayed, end-of-day,
  historical and corporate market data, and restricts redistribution to the relevant agreement.
  NSE's EOD/historical product is therefore treated as a contracted feed, not a scrape target.
- BSE's Self Data Feed similarly offers contracted equity, delayed, corporate and index data.
  MF Pulse may connect it only after the chosen plan permits storage and display to end users.
...
| Live/delayed stock prices and corporate-action-adjusted historical prices | NSE Data & Analytics EOD/historical products, BSE Self Data Feed, or an exchange-authorised vendor | Contract/subscription required for commercial display and redistribution (**OBSERVED** in the NSE Data Sharing & Usage Policy, NSE EOD/historical subscription page and BSE Self Data Feed) | Requires the relevant agreement | Must be explicit in the selected agreement | Contracted feed/API | Per licence tier | 🟠 Product path is ready, feed is not contracted. Company pages render the complete available `company_valuation_snapshots` history with source and observation count, but do not call a partial series “lifetime”, do not imply total return from unadjusted closes, and show a licence-gated empty state when fewer than two observations exist. |
