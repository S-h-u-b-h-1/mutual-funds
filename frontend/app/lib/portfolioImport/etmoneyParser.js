...
// ET Money's mutual fund holdings export. No verified real sample was available while building
...
// add ET Money-specific aliases/overrides here once a real export confirms the actual column
// names.
export function parseEtMoneyCsv(text) {
  const { rows, warnings } = parseCsvWithAliases(text, COMMON_ALIASES);
  return { rows, warnings, source: "etmoney" };
}
