...
// `tip` is optional per-item (Phase 6 — explain everything): when present, renders the existing
// MetricTooltip next to the label. Additive only — items without `tip` render exactly as before.
    ...
    <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-line bg-line sm:grid-cols-3 lg:grid-cols-6">
      {items.map((it, i) => (
        <div key={i} className="bg-bg px-4 py-3.5">
          <div className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-[0.09em] text-ink-faint">
            {it.label}
            {it.tip && <MetricTooltip>{it.tip}</MetricTooltip>}
          </div>
          <div
