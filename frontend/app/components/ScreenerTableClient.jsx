...
// Only set when the fund was matched from acquired factsheet data (see funds/page.js's
// AUM/expense filters) — most funds have neither, so this stays silent rather than showing
// "Missing" for the 93% of the universe with no factsheet yet.
function factsheetTag(fund) {
  const parts = [];
  if (fund._aum != null) parts.push(`₹${fund._aum.toLocaleString("en-IN")} Cr AUM`);
  if (fund._expense != null) parts.push(`${fund._expense}% expense`);
