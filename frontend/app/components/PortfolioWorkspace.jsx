...
export default function PortfolioWorkspace() {
  ...
  return <div className="space-y-8">
    ...
    {report && <section className="space-y-6" aria-label="Portfolio report"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Portfolio value",money(report.portfolioSummary.totalValue)],["Health score",report.portfolioSummary.healthScore == null ? "Incomplete" : `${report.portfolioSummary.healthScore}/100`],["Diversification",`${report.diversification.score}/100`],["Concentration",`${report.concentration.score}/100`]].map(([label,value]) => <div key={label} className="research-surface p-4"><div className="eyebrow">{label}</div><div className="financial-number mt-2 text-xl font-semibold text-ink">{value}</div></div>)}</div><div className="grid gap-4 lg:grid-cols-2"><AllocationList title="AMC allocation" items={report.allocations.amc} /><AllocationList title="Category allocation" items={report.allocations.category} /></div><div className="grid gap-4 lg:grid-cols-2"><section className="research-surface p-5"><div className="eyebrow">Overlap evidence</div><div className="mt-4 space-y-3 text-sm text-ink-muted">{report.overlap.duplicateAmcs.length ? report.overlap.duplicateAmcs.map((item) => <p key={item.amc}><b className="text-ink">{item.amc}</b> appears across {item.funds.length} funds · {item.totalWeightPct?.toFixed(1)}% weight</p>) : <p>No duplicate AMC concentration detected.</p>}{report.overlap.duplicateStocks.slice(0,5).map((item) => <p key={item.stock}><b className="text-ink">{item.stock}</b> · {item.totalExposurePct?.toFixed(1)}% estimated disclosed exposure</p>)}</div></section><section className="research-surface p-5"><div className="eyebrow">Known limitations</div><ul className="mt-4 space-y-2 text-sm text-ink-muted">{(report.weaknesses || []).map((item) => <li key={item}>— {item}</li>)}{unresolved.length > 0 && <li>— {unresolved.length} holding records could not be resolved.</li>}</ul></section></div><section className="research-surface p-5"><div className="eyebrow">Research opportunities</div><div className="mt-4 grid gap-3 sm:grid-cols-2">{(report.researchOpportunities || []).map((item, index) => <article key={index} className="rounded-xl border border-line bg-surface-2 p-4 text-sm leading-6 text-ink-muted">{typeof item === "string" ? item : item.reason || item.question || JSON.stringify(item)}</article>)}</div><a href="/advisor" className="mt-5 inline-flex min-h-10 items-center rounded-xl border border-line px-4 text-xs font-semibold text-ink">Review with an advisor</a></section></section>}
  </div>;
}
