...
export default function SchemePicker({ value, onChange, label = "Choose a scheme", required = true, id = "scheme-picker", allowedCodes = null }) {
  ...
  return <div className="relative">
    ...
    <input id={id} className="mt-2 min-h-12 w-full rounded-2xl border border-line bg-bg px-4 text-sm text-ink outline-none focus:border-accent focus:ring-2 focus:ring-accent/15" value={query} onChange={(event) => { setQuery(event.target.value); if (value) onChange(null); }} placeholder="Search by fund name, AMC or category" required={required} autoComplete="off" />
