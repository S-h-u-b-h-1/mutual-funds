...
export default function OnboardingFlow() {
  ...
  const content = useMemo(() => ({
    profile: ["Tell us about you", "Used to establish suitability and complete your investor record."], mobile: ["Verify your mobile", "Demo OTP: 123456 — sandbox only. No real verification or delivery."], email: ["Verify your email", "Demo OTP: 123456 — sandbox only. No real verification or delivery."],
    ...
    {loading ? <LoadingCards /> : error ? <ErrorCard message={error} retry={refresh} /> : <div className="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <Card className="overflow-hidden"><div className="border-b border-line p-5 sm:p-7"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-semibold text-ink-faint">Step {index+1} of {steps.length} · {data?.compliance?.completed || 0} of {data?.compliance?.total || 9} checks complete</div><div className="mt-3 h-2 max-w-xl overflow-hidden rounded-full bg-surface-2"><div className="h-full rounded-full bg-accent transition-all" style={{ width: `${data?.compliance?.percent || 0}%` }} /></div><h2 className="mt-4 text-2xl font-semibold tracking-[-.04em] text-ink">{stepName}</h2></div>{item && <StatusPill status={item.status} />}</div></div>
        <form onSubmit={submit} className="p-5 sm:p-7">
          ...
          {stepId === "mobile" && <div className="grid max-w-md gap-5"><Field label="Mobile number" name="phoneNumber" type="tel" placeholder="+91 98765 43210" value={form.phoneNumber} onChange={update}/><Field label="Verification code" name="otp" inputMode="numeric" placeholder="Demo OTP: 123456 — sandbox only" value={form.otp} onChange={update}/></div>}
          {stepId === "email" && <div className="max-w-md"><Field label="Verification code" name="otp" inputMode="numeric" placeholder="Demo OTP: 123456 — sandbox only" value={form.otp} onChange={update}/></div>}
