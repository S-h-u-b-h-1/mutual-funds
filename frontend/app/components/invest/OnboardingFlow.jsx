...
const steps = [
  ["profile", "Profile"], ["mobile", "Contact"], ["pan", "PAN"], ["identity", "Identity"],
...
export default function OnboardingFlow() {
  ...
  const content = useMemo(() => ({
    profile: ["Tell us about you", "Used to establish suitability and complete your investor record."], mobile: ["Verify your contact", "In this mock environment, use OTP 123456. Real delivery is not active."],
    ...
    {loading ? <LoadingCards /> : error ? <ErrorCard message={error} retry={refresh} /> : <div className="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <Card className="overflow-hidden"><div className="border-b border-line p-5 sm:p-7"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-semibold text-ink-faint">Step {index+1} of {steps.length} · about 3 minutes</div><h2 className="mt-1 text-2xl font-semibold tracking-[-.04em] text-ink">{stepName}</h2></div>{item && <StatusPill status={item.status} />}</div></div>
        <form onSubmit={submit} className="p-5 sm:p-7">
          ...
          {stepId === "mobile" && <div className="max-w-md"><Field label="Verification code" name="otp" inputMode="numeric" placeholder="Use 123456 in mock mode" value={form.otp} onChange={update}/></div>}
          ...
          {stepId === "fatca" && <label className="flex max-w-2xl gap-3 rounded-2xl bg-surface-2 p-5 text-sm leading-6 text-ink-muted"><input type="checkbox" name="fatca" checked={form.fatca} onChange={update} required className="mt-1 h-5 w-5 accent-[rgb(var(--color-brand))]"/><span>I confirm that my tax-residency details are accurate and I agree to provide additional information if required.</span></label>}
          ...
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-line pt-5"><button type="button" disabled={index===0} onClick={()=>{setIndex(i=>Math.max(0,i-1));setNotice("");}} className="min-h-11 rounded-full border border-line px-5 text-sm font-semibold text-ink disabled:opacity-35">Back</button><div className="flex gap-2"><button type="submit" disabled={saving} className="min-h-11 rounded-full bg-ink px-6 text-sm font-semibold text-bg disabled:opacity-50">{saving ? "Saving…" : "Save step"}</button>{index < steps.length-1 && <button type="button" onClick={()=>{setIndex(i=>Math.min(steps.length-1,i+1));setNotice("");}} className="min-h-11 rounded-full bg-accent px-6 text-sm font-semibold text-white">Next</button>}</div></div>
        </form>
      </Card>
    </div>}
  </InvestShell>;
