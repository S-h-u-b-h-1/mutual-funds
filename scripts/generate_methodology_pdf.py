...
def header_footer(canvas, doc):
    ...
    if doc.page > 1:
        ...
        canvas.drawRightString(PAGE_W - 18 * mm, PAGE_H - 11 * mm, "Ranking and analysis methodology - Version 3.1")
    canvas.setStrokeColor(LINE)
...
def build_story():
    story = []
    cover = Table([[p("MF PULSE", "Eyebrow")], [p("Ranking and analysis methodology", "TitleWhite")], [p("How fund, AMC and portfolio conclusions are calculated, explained, confidence-capped and compared.", "SubWhite")], [Spacer(1, 34 * mm)], [p("VERSION 3.1  |  EVIDENCE-AWARE ANALYSIS", "SubWhite")]], colWidths=[PAGE_W - 36 * mm], rowHeights=[12 * mm, 40 * mm, 25 * mm, 60 * mm, 12 * mm])
    ...
    story += [Spacer(1, 6 * mm), p("Methodology status", "H2x"), p("Version 3.1 adds the seven-step portfolio evaluation strategy and evidence-rich strength/weakness explanations. Future versions should add aligned long-window risk series, official TRI histories, debt analytics, broader factsheet coverage and published walk-forward model validation.", "Callout")]
    return story
