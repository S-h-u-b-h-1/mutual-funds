...
def build_snapshot(nifty_raw: bytes, bse_raw: bytes) -> dict:
    ...
    return {
        ...
        "indices": {
            ...
            "BSE100": {
                ...
                "identifierCoverage": {"isin": 0, "nseSymbol": 0, "bseCode": len(bse)},
...
def main() -> int:
    ...
    if bool(args.nifty_file) != bool(args.bse_file):
        parser.error("--nifty-file and --bse-file must be supplied together")
    ...
    snapshot = build_snapshot(nifty_raw, bse_raw)
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(snapshot, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(
        f"Collected {snapshot['indices']['NIFTY50']['constituentCount']} NIFTY 50 and "
        f"{snapshot['indices']['BSE100']['constituentCount']} BSE 100 constituents -> {OUTPUT}"
    )
    return 0
